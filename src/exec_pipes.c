...
static void	child_io(t_cmd *cmd, int **fd, int i, int num_cmds)
{
	if (cmd->infile || cmd->here_doc)
	{
		...
		dup2(cmd->fd_in, STDIN_FILENO); //// protection
		close(cmd->fd_in);
	}
	else if (i > 0)
	{
		close(fd[i - 1][1]);
		dup2(fd[i - 1][0], STDIN_FILENO); //// protection
		close(fd[i - 1][0]);
	}
	if (cmd->outfile)
	{
		...
		dup2(cmd->fd_out, STDOUT_FILENO); //// protection
		close(cmd->fd_out);
	}
	else if (i < num_cmds - 1)
	{
		close(fd[i][0]);
		dup2(fd[i][1], STDOUT_FILENO); //// protection
		close(fd[i][1]);
	}
}
